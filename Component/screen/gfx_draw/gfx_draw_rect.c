...
static void gfx_draw_rect_main_mid(const gfx_area_t * mask, const gfx_style_t * style);
static void gfx_draw_rect_border_straight(const gfx_area_t * mask, const gfx_style_t * style);
...
static void gfx_draw_rect_border_straight(const gfx_area_t* mask, const gfx_style_t* style)
{
    ...
    if(part & GFX_BORDER_TOP) {
        work_area.x1 = mask->x1;
        work_area.x2 = mask->x2;
        work_area.y1 = mask->y1;
        work_area.y2 = mask->y1 + bwidth;
        gfx_draw_fill(&work_area, color);
    }

    if(part & GFX_BORDER_RIGHT) {
        work_area.x1 = mask->x2 - bwidth;
        work_area.x2 = mask->x2;
        work_area.y1 = mask->y1 + (part & GFX_BORDER_TOP ? bwidth + 1 : 0);
        work_area.y2 = mask->y2 - (part & GFX_BORDER_BOTTOM ? bwidth + 1 : 0);
        gfx_draw_fill(&work_area, color);
    }

    if(part & GFX_BORDER_LEFT) {
        work_area.x1 = mask->x1;
        work_area.x2 = mask->x1 + bwidth;
        work_area.y1 = mask->y1 + (part & GFX_BORDER_TOP ? bwidth + 1 : 0);
        work_area.y2 = mask->y2 - (part & GFX_BORDER_BOTTOM ? bwidth + 1 : 0);
        gfx_draw_fill(&work_area, color);
    }
    ...
    if(part & GFX_BORDER_BOTTOM) {
        work_area.x1 = mask->x1;
        work_area.x2 = mask->x2;
        work_area.y1 = mask->y2 - bwidth;
        work_area.y2 = mask->y2;
        gfx_draw_fill(&work_area, color);
    }
}
